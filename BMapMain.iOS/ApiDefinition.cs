...
namespace BMapMain
{
	...
	interface BMKAnnotationView
	{
		...
		//[iOS(3, 2)]
		[Introduced(PlatformName.iOS, PlatformArchitecture.All, "Platform.IOS_3_2")]
		[Export("draggable")]
		...
		[Introduced(PlatformName.iOS, PlatformArchitecture.All, "Platform.IOS_3_2")]
		[Export("dragState")]
		BMKAnnotationViewDragState DragState { get; set; }
	}
	...
	interface BMKOverlayView
	{
		...
		//unsafe void DrawMapRect(BMKMapRect mapRect, double zoomScale, IntPtr context);
		void DrawMapRect(BMKMapRect mapRect, double zoomScale, CGContext context);
	...
	interface BMKMapView
	{
        ...
        NSObject[] GlPointsForMapPoints(ref BMKMapPoint mapPoints, nuint count);
	...
	interface BMKMapView_IndoorMapAPI
	{
		...
		[Export("setBaseIndoorMapEnabled:")]
		void SetBaseIndoorMapEnabled(bool enabled);

		// @property (assign, nonatomic) BOOL showIndoorMapPoi;
		[Export("setShowIndoorMapPoi:")]
		void SetShowIndoorMapPoi(bool show);
	...
	interface BMKMapView_AnnotationAPI
	{
        ...
        [Export("addAnnotation:")]
        void AddAnnotation(BMKMultiPoint annotation);
        ...
        [Export("removeAnnotation:")]
        void RemoveAnnotation(BMKMultiPoint annotation);
	...
	interface BMKMapViewDelegate
	{
		...
		void MapViewDidDeselectAnnotationView(BMKMapView mapView, BMKAnnotationView view);

        //[Export("mapView:didDeselectAnnotationView:")]
        //void MapViewDidDeselectAnnotationView(BMKMapView mapView, BMKPinAnnotationView view);
		...
		void MapViewAnnotationViewDidChangeDragStateFromOldState(BMKMapView mapView, BMKAnnotationView view, BMKAnnotationViewDragState newState, BMKAnnotationViewDragState oldState);
		...
		void MapViewOnClickedBMKOverlayView(BMKMapView mapView, BMKOverlayView overlayView);

		// @optional -(void)mapView:(BMKMapView *)mapView onClickedMapPoi:(BMKMapPoi *)mapPoi;
		[Export("mapView:onClickedMapPoi:")]
		void MapViewOnClickedMapPoi(BMKMapView mapView, BMKMapPoi mapPoi);
		...
		void MapViewOnClickedMapBlank(BMKMapView mapView, CLLocationCoordinate2D coordinate);

		// @optional -(void)mapview:(BMKMapView *)mapView onDoubleClick:(CLLocationCoordinate2D)coordinate;
		[Export("mapview:onDoubleClick:")]
		void MapViewOnDoubleClick(BMKMapView mapView, CLLocationCoordinate2D coordinate);

		// @optional -(void)mapview:(BMKMapView *)mapView onLongClick:(CLLocationCoordinate2D)coordinate;
		[Export("mapview:onLongClick:")]
		void MapViewOnLongClick(BMKMapView mapView, CLLocationCoordinate2D coordinate);
		...
		void MapViewOnForceTouch(BMKMapView mapView, CLLocationCoordinate2D coordinate, nfloat force, nfloat maximumPossibleForce);
		...
		void MapviewBaseIndoorMapWithInBaseIndoorMapInfo(BMKMapView mapView, bool flag, BMKBaseIndoorMapInfo info);
	}
	...
	interface BMKArcline : BMKOverlay
	{
		...
		unsafe BMKArcline ArclineWithPoints(ref BMKMapPoint points);

		// +(BMKArcline *)arclineWithCoordinates:(CLLocationCoordinate2D *)coords;
		[Static]
		[Export("arclineWithCoordinates:")]
		unsafe BMKArcline ArclineWithCoordinates(ref CLLocationCoordinate2D coords);

		// -(BOOL)setArclineWithPoints:(BMKMapPoint *)points;
		[Export("setArclineWithPoints:")]
		unsafe bool SetArclineWithPoints(ref BMKMapPoint points);

		// -(BOOL)setArclineWithCoordinates:(CLLocationCoordinate2D *)coords;
		[Export("setArclineWithCoordinates:")]
		unsafe bool SetArclineWithCoordinates(ref CLLocationCoordinate2D coords);
	}
	...
	interface BMKOverlayPathView
	{
		...
		void ApplyStrokePropertiesToContext(CGContext context, double zoomScale);
		...
		void ApplyFillPropertiesToContext(CGContext context, double zoomScale);
